...
void PhysicsProcessor::SetSparsity(
...
void PhysicsProcessor::AddLoadContribution(
    const std::vector<double> &load_vector, const Matrix<double> &N,
...
void PhysicsProcessor::Scatter(const std::vector<size_t> &dofs_tags,
  ...
  for (size_t i_dof_idx = 0; i_dof_idx < dofs_tags.size(); i_dof_idx++) {
    ...
    for (size_t j_dof_idx = i_dof_idx; j_dof_idx < dofs_tags.size();
         j_dof_idx++) {
      const auto &value = element_stiffness.selfadjointView<Eigen::Upper>()(
          i_dof_idx, j_dof_idx);
