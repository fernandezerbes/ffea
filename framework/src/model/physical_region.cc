...
void PhysicalRegion::Contribute(StiffnessTerm& term, Element& element,
                                size_t integration_point_idx) const {}

void PhysicalRegion::Contribute(MassTerm& term, Element& element,
                                size_t integration_point_idx) const {}

void PhysicalRegion::Contribute(DampingTerm& term, Element& element,
                                size_t integration_point_idx) const {}

void PhysicalRegion::Contribute(RhsTerm& term, Element& element,
                                size_t integration_point_idx) const {}

void PhysicalRegion::ComputeLoadContribution(Element& element, VectorialFunction load,
...
void Domain::Contribute(StiffnessTerm& term, Element& element, size_t integration_point_idx) const {
  const auto& global_coords = element.global_coords(integration_point_idx);
...
void Domain::Contribute(RhsTerm& term, Element& element, size_t integration_point_idx) const {
  Vector<double> contribution = Vector<double>::Zero(element.number_of_dofs());
  ComputeLoadContribution(element, source_, integration_point_idx, contribution);
  term.element_vector() += contribution;
}

DomainBoundary::DomainBoundary(std::vector<Element>& elements, VectorialFunction load)
    : PhysicalRegion(elements), load_(load) {}

void DomainBoundary::Contribute(RhsTerm& term, Element& element,
                                size_t integration_point_idx) const {
  Vector<double> contribution = Vector<double>::Zero(element.number_of_dofs());
  ComputeLoadContribution(element, load_, integration_point_idx, contribution);
  term.element_vector() += contribution;
}
