...
std::vector<ffea::NodalValuesGroup> DerivedVariableProcessor::ExtractValuesOfAllElementsPerNode(
  ...
  for (size_t node_idx = 0; node_idx < raw_values.size(); node_idx++) {
    ...
    for (const auto &values : values_group) {
      for (size_t value_idx = 0; value_idx < values_per_node(); value_idx++) {
        ...
        avg_values[global_value_idx] += values[value_idx] / static_cast<double>(group_size);
      }
    }
  }
