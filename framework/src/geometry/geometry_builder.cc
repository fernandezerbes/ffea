...
GeometryBuilder::GeometryBuilder(const GeometricEntityFactory &factory) : factory_(factory) {}
...
void GeometryFromFileBuilder::AddEntitiesGroups(Geometry &geometry) {
  for (const auto &entity_group : data_.geometric_entity_groups()) {
    geometry.RegisterGeometricEntityGroup(entity_group.name(), entity_group.entity_tags());
  }
}
