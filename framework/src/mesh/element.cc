...
Coordinates &Element::node_coords(size_t node_idx) const { return entity_.node_coords(node_idx); }

std::vector<size_t> Element::node_tags() const { return entity_.node_tags(); }

size_t Element::node_tag(size_t local_node_idx) const { return entity_.node_tag(local_node_idx); }
...
Vector<double> Element::EvaluateNormalVector(const Coordinates &local_coords) const {
