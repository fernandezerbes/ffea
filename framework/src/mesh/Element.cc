...
Element::Element(const std::vector<Node *> &nodes,
                 std::unique_ptr<ShapeFunctions> shape_functions)
    : nodes_(nodes), shape_functions_(std::move(shape_functions)) {}
...
size_t Element::number_of_nodes() const { return nodes_.size(); }
