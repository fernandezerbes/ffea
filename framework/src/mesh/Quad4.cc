...
Quad4::Quad4(const std::vector<Node *> &nodes)
    : Element(nodes, std::make_unique<Linear2DShapeFunctions>()) {}
...
Eigen::MatrixXd Quad4::EvaluateJacobian(
  ...
  const auto &shape_functions_derivatives =
      EvaluateShapeFunctionsDerivatives(local_coordinates);
  for (size_t node_index = 0; node_index < nodes_.size(); node_index++) {
    const auto &node = nodes_[node_index];
    const auto &coords = node->coordinates();
    jacobian(0, 0) +=
        coords.get(0) * shape_functions_derivatives(0, node_index);
    jacobian(0, 1) +=
        coords.get(1) * shape_functions_derivatives(0, node_index);
    jacobian(1, 0) +=
        coords.get(0) * shape_functions_derivatives(1, node_index);
    jacobian(1, 1) +=
        coords.get(1) * shape_functions_derivatives(1, node_index);
  }
  return jacobian;
}
